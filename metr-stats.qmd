...
* Tasks of the same human time vary a lot in difficulty. This matters because at any given date, the typical horizon is roughly an order of magnitude longer than the marginal one. The marginal (random task) version is what's practically relevant, but it's less impressive than the typical version METR uses.
