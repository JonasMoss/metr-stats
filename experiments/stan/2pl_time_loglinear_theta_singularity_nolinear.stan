...
parameters {
  ...
  // Pure finite-time singularity trend for ability:
  //   θ(d) = gamma0 + c / (t_star - x)^alpha_sing
  real gamma0;
  real<lower=0> c_sing;
...
model {
  // Trend priors: keep it sampling-friendly.
  gamma0 ~ normal(0, 1.5);
  ...
  // Ability "prior" as a regression likelihood for dated models (includes anchors).
  for (i in 1:I) {
    if (has_date[i] == 1) {
      real denom = t_star - x_date[i];
      real mu = gamma0 + c_sing / pow(denom, alpha_sing);
...
generated quantities {
  ...
  for (i in 1:I) {
    if (has_date[i] == 1) {
      real denom = t_star - x_date[i];
      mu_theta[i] = gamma0 + c_sing / pow(denom, alpha_sing);
    } else {
      mu_theta[i] = 0;
    }
  }
